...
fig, ax = plt.subplots(figsize=(9.0,2.50), dpi=400)
ax.set_xlim(0,16); ax.set_ylim(0,4.44); ax.axis('off')

cards = [
 ("1","DLVO","Does a barrier exist?","IEP shifted to 4.94 by the caseinate\n17–49 kT at 10 mM — real stability\ncollapses with salt; CCC at pH 4"),
 ("2","Docking","Which contacts form?","no specific site among five milk proteins\ninterfaces of only 3–6 residues\n→ non-specific, multivalent corona"),
 ("3","Coarse-grained MD","Does the interface hold?","next: a 12 nm patch, ~2 × 10⁴ beads\ncontact persistence over time\na steric term for extended DLVO"),
]
CW, CH = 4.55, 2.80
CY0, CY1 = 1.00, 1.00+CH
...
for (num,title,q,out), cx in zip(cards, centers):
    ...
    ax.text(cx, CY1-0.92, q, ha='center', va='center', fontsize=9.0, style='italic', color=DARK, zorder=4)
    ax.text(cx, CY1-1.78, out, ha='center', va='center', fontsize=7.6, color=GREY, zorder=4, linespacing=1.6)

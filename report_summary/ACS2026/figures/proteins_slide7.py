...
HYDC="#E8A33D"; POLC="#C9C9C9"; CHGC="#2E6DA4"; DARK="#000000"; GREY="#000000"; RED="#E21833"
...
fig=plt.figure(figsize=(9.0,2.812), dpi=400)
bg=fig.add_axes([0,0,1,1]); bg.set_xlim(0,16); bg.set_ylim(0,5.00); bg.axis('off')
...
bg.annotate("κ-casein\nbrush", xy=(cx-0.80, IMG_CY+0.32), xytext=(cx-1.95, IMG_CY+0.62),
            fontsize=8.5, color=CHGC, ha='center', va='center', linespacing=1.3,
...
for cx,(n,d) in zip(cols, caps):
    bg.text(cx, 3.26, n, fontsize=11.5, fontweight='bold', color=DARK, ha='center', va='top')
    bg.text(cx, 2.88, d, fontsize=9.5, color=GREY, ha='center', va='top', linespacing=1.3)

# --- composition bars ---
ax=fig.add_axes([fx(1.75), fy(0.50), fx(12.9), fy(1.56)])
...
for yy,h,p,c in zip(y,hyd,pol,chg):
    ax.text(min(h+p+c-0.6, 99.4), yy, f"{c:.0f}%", fontsize=9.0, color='white',
            va='center', ha='right', fontweight='bold')
ax.set_yticks(y); ax.set_yticklabels(short, fontsize=9.0)
ax.set_xlim(0,100); ax.set_xticks([0,25,50,75,100])
ax.set_xticklabels(["0","25","50","75","100%"], fontsize=9.0)
ax.tick_params(length=2, pad=1.5)
for sp in ['top','right','left']: ax.spines[sp].set_visible(False)
# label the three classes inside the top bar
ytop=y[0]
ax.text(hyd[0]/2, ytop, "hydrophobic", fontsize=9.0, color='white', ha='center', va='center', fontweight='bold')
ax.text(hyd[0]+pol[0]/2, ytop, "polar", fontsize=9.0, color='#000000', ha='center', va='center', fontweight='bold')
ax.text(hyd[0]+pol[0]+chg[0]/2, ytop, "charged", fontsize=9.0, color='white', ha='center', va='center', fontweight='bold')
ax.text(87.0, y[-1], "only 2% charged", fontsize=10.0, color=RED, ha='right', va='center', fontweight='bold')
